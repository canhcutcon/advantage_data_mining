...
\begin{frame}{Ví dụ chạy xuyên suốt}
\centering
\begin{tabular}{cl}
\toprule
...
\begin{itemize}\itemsep0.6em
  ...
  \item \textbf{19 assert} đối chiếu từng con số bài giảng --- {\color{okgreen}\textbf{100\% ĐẠT}}
  \item[]
  \item Phát hiện thiếu sót trong slide gốc: FIM liệt kê 10, \textbf{đúng là 11}\\
        (sót \{pasta, orange, cake\}, $sup = 2$)
  \item \{pasta, cake\}: $lift = 1.0$, $bond = 0.5$ $\Rightarrow$ xác nhận mẫu giả bằng số
\end{itemize}
\note{Điểm tự hào: kiểm chứng tự động đủ chặt để bắt được lỗi của chính tài liệu
gốc --- danh sách chữ thiếu một itemset mà hình lattice lại vẽ đúng. Bảng độ đo cặp
cũng xác nhận bằng số trực giác mẫu giả ở slide 9. (1:00)
