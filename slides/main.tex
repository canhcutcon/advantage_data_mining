...
nó sẽ quay lại ở phần mẫu giả. (0:45) Chuyển: ``Trước khi đi vào chi tiết, đây là
toàn cảnh dòng chảy bài toán trên chính ví dụ này.''}
\end{frame}

% ============ Slide 6: Full flow on the running example ============
\begin{frame}{Toàn cảnh: dòng chảy bài toán trên ví dụ}
\centering
\begin{tikzpicture}[
  font=\small,
  box/.style={draw=primary, thick, rounded corners=3pt, align=center,
              minimum height=1.15cm, inner sep=6pt, fill=primary!8},
  res/.style={align=center, font=\footnotesize, text=darktext},
  lbl/.style={font=\scriptsize, text=accent},
  arr/.style={-{Stealth[length=2.6mm]}, very thick, accent},
  node distance=0.45cm and 0.72cm
]
\node[box] (db) {\textbf{CSDL giao dịch}\\T1--T4, 6 mục};
\node[box, right=of db] (mine) {\textbf{Khai thác mẫu hiếm}\\AprioriRare / AprioriInverse};
\node[box, right=of mine] (corr) {\textbf{Lọc tương quan}\\bond, all-confidence};
\node[box, right=of corr, draw=okgreen, fill=okgreen!10] (out) {\textbf{CORI}\\hiếm + gắn kết};
\draw[arr] (db) -- node[lbl, above]{$minsup$} (mine);
\draw[arr] (mine) -- node[lbl, above]{$minbond$} (corr);
\draw[arr] (corr) -- (out);
\node[res, below=of db] {\{bread\}: $sup{=}1$\\pasta: mọi giao dịch};
\node[res, below=of mine] {mRI: \{bread\}, \{lemon, cake\}\\PRI: \{bread\}, \dots};
\node[res, below=of corr] {loại \{pasta, cake\}\\($lift{=}1$, $bond{=}0.5$)};
\node[res, below=of out] {\textbf{\{bread\}, \{cake\},}\\\textbf{\{orange, cake\}}};
\end{tikzpicture}
\vspace{0.9em}
\begin{itemize}
  \item Một dòng chảy: \textbf{định nghĩa hiếm} $\to$ \textbf{thuật toán} $\to$ \textbf{độ đo lọc mẫu giả} $\to$ tri thức tin cậy
  \item Các phần sau đi sâu vào từng khối trên chính ví dụ này
\end{itemize}
\note{Bản đồ toàn bài trên chính ví dụ 4 giao dịch: từ CSDL thô, qua ngưỡng minsup
để định nghĩa và khai thác vùng hiếm (mRI, PRI), rồi qua độ đo tương quan để loại
mẫu giả như \{pasta, cake\}, và hội tụ ở CORI --- vừa hiếm vừa gắn kết. Mỗi khối
tương ứng một phần tiếp theo; khán giả có thể định vị mình đang ở đâu. (0:40)
Chuyển: ``Bắt đầu từ khối đầu tiên: hiếm nghĩa là gì?''}
\end{frame}

% ============ Slide 7: Three definitions ============
...
xác nhận bằng số trực giác mẫu giả ở slide ``Mẫu giả''. (1:00)
Chuyển: ``Trên dữ liệu thật thì các đánh đổi lý thuyết hiện ra thế nào?''}
\end{frame}

% ============ Slide 15: Exp A (figure) ============
...
biến, đúng như dự báo ở slide AprioriRare. Đây là lý do các nghiên cứu mới dùng metaheuristic
hoặc bit dọc để né duyệt vét cạn. (1:00)}
\end{frame}

% ============ Slide 16: Exp B & C (figure) ============
