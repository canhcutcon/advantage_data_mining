...
bond = 1 mà CORI phát hiện. Trên cơ sở chọn CORI làm phương pháp trọng tâm, báo cáo
**đề xuất cải tiến CORI+**: dùng cận trên all-confidence ($bond \le allconf$) để né phần
lớn phép hợp DTID-List tốn kém — giữ nguyên kết quả (đã kiểm chứng trùng khớp CORI) nhưng
né 96–98% phép hợp và tăng tốc 4.2×–5.4× trên Mushroom. Báo cáo khép lại bằng tổng quan
bốn dòng nghiên cứu 2021–2026: mở rộng biểu diễn dữ liệu, kết hợp tiện ích, bảo vệ quyền
riêng tư và các ứng dụng y tế/công nghiệp.
...
4. **Chọn CORI làm phương pháp trọng tâm và đề xuất cải tiến CORI+** (cận trên
   all-confidence cho bond), kiểm chứng tính đúng đắn và đo tăng tốc trên Mushroom.
5. Tổng quan hướng nghiên cứu giai đoạn 2021–2026 của lĩnh vực.
...
Trong số 22 nguồn được khảo sát, báo cáo chọn **sáu bài báo nền tảng** dưới đây làm
trục phân tích sâu của Chương 3 — đây là các công trình *gốc* định nghĩa nên ba thuật
toán và các độ đo mà báo cáo cài đặt, kiểm chứng và cải tiến (bảng 2.3).

| # | Bài báo (năm) | Đóng góp cốt lõi | Phương pháp | Hạn chế / khe cải tiến |
|---|---|---|---|---|
| [3] | Omiecinski, TKDE 2003 | Độ đo **bond**, **all-confidence**; tính null-invariant | Độ đo trên support hội/tuyển | Chỉ là độ đo, chưa kèm thuật toán hiếm |
| [4] | Koh & Rountree, PAKDD 2005 | Định nghĩa **PRI**, thuật toán **AprioriInverse** | Loại item phổ biến từ đầu, Apriori hai ngưỡng | Bỏ sót mẫu hiếm lai (item hiếm + phổ biến) |
| [5] | Szathmary et al., ICTAI 2007 | Định nghĩa **mRI**, thuật toán **AprioriRare** | Duyệt theo mức tới biên hiếm | Phải duyệt toàn bộ vùng phổ biến |
| [6] | Wu et al., DMKD 2010 | **Khung độ đo thống nhất**, hệ thống hóa null-invariance | Phân tích lý thuyết các độ đo | Cho cặp item, chưa tích hợp khai thác hiếm |
| [7] | Bouasker & Ben Yahia, SAC 2015 | Thuật toán **CORI** (hiếm + tương quan) | Eclat + TID/DTID-List, cắt tỉa kép | Dựng DTID-List (phép hợp) tốn kém → **mục 3.7 cải tiến** |
| [8] | Koh & Ravana, TKDD 2016 | **Khảo sát/taxonomy** mẫu hiếm | Tổng hợp định nghĩa & thuật toán | Dừng ở giai đoạn đầu, chưa có hướng 2021–2026 |

*Bảng 2.3 — Sáu bài báo nền tảng được phân tích sâu trong báo cáo. Cột cuối nêu rõ
khe cải tiến mà mục 3.7 khai thác đối với CORI [7].*

Bổ sung cho lõi nền tảng đó, khảo sát của Koh & Ravana [8] đã hệ thống hóa các định nghĩa
và thuật toán khai thác mẫu hiếm giai đoạn đầu. Trong 5 năm gần đây, lĩnh vực phát triển theo bốn
dòng chính:
...
Chương này phân tích ba thuật toán nền tảng (AprioriRare, AprioriInverse, CORI)
để làm rõ phổ thiết kế của lĩnh vực, sau đó **chọn CORI làm phương pháp trọng tâm**
để đi sâu và đề xuất cải tiến (mục 3.7). Lý do lựa chọn: trong ba thuật toán, chỉ CORI
giải đồng thời *cả hai* yêu cầu cốt lõi của bài toán — vừa **hiếm** (ràng buộc monotone
theo support) vừa **thực sự gắn kết** (ràng buộc anti-monotone theo bond) — nên cho tập
kết quả nhỏ và giàu ngữ nghĩa nhất (xem so sánh ở bảng 3.2). AprioriRare phải duyệt toàn
bộ vùng phổ biến (đắt), còn AprioriInverse bỏ sót các mẫu hiếm lai chứa item phổ biến
(hẹp); CORI tránh được cả hai nhược điểm và còn để ngỏ một khe tối ưu rõ ràng (chi phí
dựng DTID-List) mà mục 3.7 sẽ khai thác.

| Thuật toán | Bài toán | Ràng buộc | Ưu điểm | Nhược điểm |
|---|---|---|---|---|
| AprioriRare [5] | mRI (biên hiếm) | $sup < minsup$ + tập con phổ biến | Biểu diễn biên gọn | Duyệt hết vùng phổ biến |
| AprioriInverse [4] | PRI (hiếm thuần) | 2 ngưỡng, mọi item hiếm | Không gian nhỏ, rất nhanh | Bỏ sót mẫu hiếm lai |
| **CORI [7]** (chọn) | Rare correlated | $sup < maxsup$ + $bond \ge minbond$ | Kết quả nhỏ, giàu ngữ nghĩa | Chi phí dựng DTID-List |

*Bảng 3.2 — So sánh ba thuật toán và lý do chọn CORI làm phương pháp trọng tâm.*
...
## 3.7. Đề xuất cải tiến: CORI+ với cận trên all-confidence

Phân tích chi phí của CORI chỉ ra một điểm nghẽn cụ thể: tại **mỗi** ứng viên, thuật
toán phải dựng **DTID-List** bằng phép **hợp** (union) các TID-set của thành phần để
tính $dsup$, rồi mới biết $bond = sup/dsup$ có đạt $minbond$ hay không. Trên dữ liệu
*dày* như Mushroom, các TID-set rất lớn nên phép hợp này đắt — trong khi đa số ứng viên
cuối cùng lại bị **loại** vì bond không đạt. Nói cách khác, CORI trả chi phí đắt nhất
(dựng DTID-List) *trước khi* biết ứng viên có sống sót không.

Cải tiến đề xuất — gọi là **CORI+** — dựa trên một **cận trên rẻ** của bond:

**Bổ đề 3.1.** Với mọi itemset $X$ khác rỗng:
$$bond(X) \;\le\; allconf(X) \;=\; \frac{sup(X)}{\max_{i \in X} sup(\{i\})}.$$

*Chứng minh.* Theo định nghĩa, $dsup(X) = \left|\bigcup_{i \in X} TID(\{i\})\right|$.
Vì với mọi $i \in X$ ta có $TID(\{i\}) \subseteq \bigcup_{j \in X} TID(\{j\})$, suy ra
$dsup(X) \ge \max_{i \in X} |TID(\{i\})| = \max_{i \in X} sup(\{i\})$. Do
$bond(X) = sup(X)/dsup(X)$ và mẫu số $dsup(X)$ không nhỏ hơn $\max_i sup(\{i\})$, ta được
$bond(X) \le sup(X)/\max_i sup(\{i\}) = allconf(X)$. $\blacksquare$

**Hệ quả (quy tắc cắt tỉa).** Nếu $allconf(X) < minbond$ thì chắc chắn
$bond(X) < minbond$, nên $X$ và mọi tập cha của nó đều bị loại — **mà không cần dựng
DTID-List**. Điểm mấu chốt: $allconf(X)$ chỉ cần $sup(X)$ (đã có sẵn từ phép **giao**
TID-List khi mở rộng, vốn rẻ hơn phép hợp) và support các item đơn $sup(\{i\})$ (tính
trước một lần). Vì vậy CORI+ thử cận trên all-confidence *trước*; chỉ những ứng viên
**vượt qua** cận trên mới phải dựng DTID-List để lấy bond chính xác.

```text
Thuat toan CORI+(D, maxsup, minbond, max_size)   # khac biet voi CORI in DAM
  sort items theo support tang dan
  single_sup[i] <- sup({i}) cho moi item        # tinh truoc mot lan
  expand(prefix, tids, dtids, start, max_single):
    for item tu start ... :
      new_tids <- tids ∩ TID(item)              # phep GIAO (re)
      s <- |new_tids|
      new_max_single <- max(max_single, single_sup[item])
      if s / new_max_single < minbond:          # ⇐ CAN TREN all-confidence
          continue                              #   cat ngay, KHONG dung DTID
      new_dtids <- dtids ∪ TID(item)            # phep HOP chi cho nut song sot
      b <- s / |new_dtids|                      # bond chinh xac
      if b < minbond: continue
      if s < maxsup: xuat (prefix+item, s, b)
      if |prefix|+1 < max_size: expand(prefix+item, new_tids, new_dtids, ...)
```

**Tính đúng đắn.** CORI+ cho kết quả **đồng nhất** với CORI: cận trên chỉ loại các ứng
viên mà CORI cũng loại (vì $allconf < minbond \Rightarrow bond < minbond$); các ứng viên
còn lại được tính $bond$ chính xác y như CORI và áp đúng hai điều kiện
$sup < maxsup$, $bond \ge minbond$. Đẳng thức kết quả được kiểm chứng tự động bằng
`assert cori_plus == cori` ở mọi ngưỡng khảo sát (mục 4.5). Như vậy CORI+ **chỉ giảm chi
phí, không đánh đổi độ chính xác** — khác hẳn các cách tăng tốc bằng xấp xỉ. Cải tiến này
nối tiếp tinh thần dùng độ đo rẻ làm cận cho độ đo đắt, và tận dụng đúng quan hệ
all-confidence ⪰ bond vốn đã trình bày ở mục 3.4–3.5.
...
- `rare_mining.py` — lớp `TransactionDB` (biểu diễn dọc với TID-set cho từng item) và
  các hàm `apriori`, `apriori_rare`, `apriori_inverse`, `cori`, `cori_plus`, `bond`,
...
- `run_improvement.py` — thực nghiệm đề xuất cải tiến CORI+ (mục 4.5): kiểm chứng
  `cori_plus == cori` và đo tăng tốc so với CORI gốc.
...
## 4.5. Thực nghiệm cải tiến: CORI+ so với CORI

Mục này kiểm chứng đề xuất CORI+ (mục 3.7) trên cùng dữ liệu UCI Mushroom
($maxsup = 0.1$, trần kích thước 4), so trực tiếp với CORI gốc. Mã trong
`code/run_improvement.py`; mỗi cấu hình đo **trung vị 5 lần chạy** để giảm nhiễu.

Trước hết, ở **mọi** ngưỡng khảo sát, lệnh `assert cori_plus == cori` đều **ĐẠT** — xác
nhận thực nghiệm rằng cải tiến *không làm thay đổi tập kết quả*, đúng như chứng minh tính
đúng đắn ở mục 3.7. Trên nền đó, bảng 4.4 đo phần chi phí tiết kiệm được.

| minbond | #Kết quả | #Hợp DTID đã làm | #Hợp né được | % né | CORI (ms) | CORI+ (ms) | Tăng tốc |
|---|---|---|---|---|---|---|---|
| 0.95 | 90 | (số ít) | (đa số) | 98.4 | 592.9 | 110.7 | ×5.35 |
| 0.90 | 93 | | | 98.4 | 602.9 | 111.8 | ×5.39 |
| 0.80 | 93 | | | 98.2 | 599.1 | 114.3 | ×5.24 |
| 0.70 | 93 | | | 98.1 | 617.5 | 119.7 | ×5.16 |
| 0.60 | 94 | | | 97.5 | 661.2 | 136.8 | ×4.83 |
| 0.50 | 104 | | | 96.1 | 794.7 | 189.4 | ×4.20 |

*Bảng 4.4 — CORI+ vs CORI trên Mushroom. "% né" là tỉ lệ ứng viên bị cận trên
all-confidence loại trước khi phải dựng DTID-List. Số liệu chi tiết (kể cả cột số phép
hợp tuyệt đối) trong `results/improvement_cori_plus.csv`.*

Nhận xét:

1. **Cận trên all-confidence rất chặt trên dữ liệu dày**: nó loại tới **96–98%** ứng
   viên *trước* khi phải dựng DTID-List. Điều này hợp lý — trên Mushroom, đa số tổ hợp
   item có bond thấp, và all-confidence (vốn $\ge$ bond) đủ nhỏ để bắt được phần lớn
   trong số đó bằng một phép chia thay vì một phép hợp tập lớn.
2. **Tăng tốc thực tế 4.2×–5.4×**, đúng bản chất: chi phí trội của CORI nằm ở phép hợp
   DTID-List, và CORI+ né được gần hết các phép hợp đó.
3. **Tăng tốc giảm dần khi nới $minbond$** (×5.4 ở 0.95 → ×4.2 ở 0.5): ngưỡng càng lỏng
   thì cận trên loại được càng ít ứng viên (% né tụt 98.4 → 96.1), nên phần tiết kiệm thu
   hẹp lại — một xu hướng nhất quán với cơ chế cắt tỉa.

![Cải tiến CORI+ so với CORI trên Mushroom](results/improvement_cori_plus.png)

*Hình 4.3 — Trái: thời gian CORI vs CORI+ theo minbond. Phải: tỉ lệ phép hợp DTID
mà cận trên all-confidence né được.*

Đây là một cải tiến nhỏ nhưng *thuần khiết về thuật toán*: dựa trên một bổ đề có chứng
minh, cho kết quả đồng nhất, và đem lại tăng tốc đo được — đúng tinh thần "dùng độ đo rẻ
chặn độ đo đắt" mà các hệ khai thác mẫu hiện đại vẫn áp dụng.
...
Về **cải tiến**, báo cáo chọn CORI làm phương pháp trọng tâm và đề xuất **CORI+**: dùng
cận trên all-confidence (Bổ đề 3.1: $bond \le allconf$) để né phần lớn phép hợp DTID-List
tốn kém. Thực nghiệm trên Mushroom cho thấy CORI+ **giữ nguyên kết quả** (assert trùng
khớp CORI ở mọi ngưỡng) nhưng né được **96–98%** phép hợp DTID và **tăng tốc 4.2×–5.4×**
— một cải tiến thuật toán có chứng minh, không đánh đổi độ chính xác.
...
python3 run_improvement.py
# - results/improvement_cori_plus.md  : bảng 4.4 (CORI+ vs CORI)
# - results/improvement_cori_plus.csv : dữ liệu thô
# - results/improvement_cori_plus.png : hình 4.3
```
